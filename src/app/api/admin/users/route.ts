...
export async function GET() {
  ...
  try {
    ...
    try {
      ...
      if (!prisma.admin) {
        console.error('Admin model not available in Prisma client. Available models:', Object.keys(prisma).filter(k => !k.startsWith('$') && !k.startsWith('_') && typeof (prisma as Record<string, unknown>)[k] === 'object'));
        return NextResponse.json([]);
      }
